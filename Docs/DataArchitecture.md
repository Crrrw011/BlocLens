...
- Account deletion/anonymisation ships through the `delete-account` Edge Function; exported Logbook delivery is CSV/PDF generated on-device. The privacy policy text is drafted in `Docs/PrivacyPolicy.md`; the final effective date and a publicly hosted policy URL remain before App Store submission. Any legally required retention beyond the anonymisation strategy must still be confirmed before production deployment.
